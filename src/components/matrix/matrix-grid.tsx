...
import type { MatrixData, MatrixCell, MatrixStatus } from "@/lib/matrix";
import { 
    Tooltip,
    TooltipContent,
    TooltipProvider,
...
import { Check, X, AlertTriangle, Minus, Clock, ShieldCheck } from "lucide-react";
            ...
            <div className="relative overflow-x-auto max-h-[80vh]">
                <table className="w-full text-left text-sm whitespace-nowrap border-collapse">
                    <thead className="bg-slate-900 border-b border-white/10 sticky top-0 z-50 shadow-md">
                        <tr>
                            <th scope="col" className="sticky left-0 z-50 bg-slate-900 p-4 font-semibold text-white border-r border-white/10 min-w-[200px]">
                                Employee
                            </th>
                            {skills.map(skill => (
                                <th key={skill.id} scope="col" className="p-4 font-medium text-slate-300 min-w-[120px] text-center border-r border-white/5 bg-slate-900">
                                    <div className="flex flex-col items-center gap-1">
                                        <span className="text-xs uppercase tracking-wider text-slate-500">{skill.code}</span>
                        ...
                        {employees.map(emp => (
                            <tr key={emp.id} className="hover:bg-white/5 transition-colors">
                                ...
                                {skills.map(skill => {
                                    ...
                                    return (
                                        <td key={skill.id} className="p-2 border-r border-white/5 text-center align-middle">
...
function CellContent({ cell }: { cell?: MatrixCell }) {
    ...
    switch (status) {
        ...
        case "extra":
            ...
            break;
        default:
            return <div className="h-full w-full opacity-20 hover:opacity-100 transition-opacity flex justify-center"><Minus className="h-3 w-3 text-slate-700" /></div>;
    }
            ...
            <Tooltip>
                <TooltipTrigger asChild>
                    <div className={cn(
                        ...
                        <div className="flex items-center gap-1">
                            {icon}
                            <span className="font-bold">{achievedLevel > 0 ? achievedLevel : "-"}</span>
                            {requiredLevel > 0 && (
                                <span className="text-[10px] opacity-70">/ {requiredLevel}</span>
                            )}
                        </div>
                    </div>
                </TooltipTrigger>
                <TooltipContent className="bg-slate-950 border border-white/10 p-3 max-w-xs">
                    <div className="space-y-2">
                        <div className="font-semibold text-white capitalize">{status.replace("_", " ")}</div>
                        <div className="text-xs text-slate-400 grid grid-cols-2 gap-x-4 gap-y-1">
                            <span>Required Level:</span>
                            <span className="text-white font-mono">{requiredLevel || "None"}</span>
                            
                            <span>Achieved Level:</span>
                            <span className="text-white font-mono">{achievedLevel || "None"}</span>
                            
                            {cell.expiresAt && (
                                <>
                                    <span>Expires:</span>
                                    <span className={cn("font-mono", status === "expired" ? "text-red-400" : "text-white")}>
                        ...
                        {requirementSources.length > 0 && (
                            <div className="pt-2 border-t border-white/10">
                                <span className="text-[10px] uppercase tracking-wider text-slate-500 font-semibold">Required By:</span>
                                <ul className="mt-1 space-y-1">
                                    {requirementSources.map((src, i) => (
                                        <li key={`${src}-${i}`} className="text-xs text-slate-300">{src}</li>
                                    ))}
                                </ul>
                            </div>
                        )}
