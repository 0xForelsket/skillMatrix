    ...
    const handleRevoke = () => {
        ...
        startTransition(async () => {
            ...
            if (result.success) {
                ...
                toast.error(typeof result.error === 'string' ? result.error : "Failed to revoke.");
            }
        });
    };
            ...
            <DialogContent className="sm:max-w-[425px] bg-slate-950 border-white/10 text-white">
                <DialogHeader>
                    ...
                    <DialogDescription className="text-slate-400">
                        Are you sure you want to revoke the certification for <strong>{skillName}</strong>?
                        This action will be logged and requires a reason.
                    </DialogDescription>
                </DialogHeader>
                <div className="grid gap-4 py-4">
                    <div className="space-y-2">
                        <Label htmlFor="reason" className="text-white">Reason for Revocation</Label>
                        <Textarea
                            id="reason"
                ...
                <DialogFooter>
                    <Button variant="ghost" onClick={() => setOpen(false)} disabled={isPending} className="text-slate-400 hover:text-white hover:bg-white/10">
                        Cancel
                    </Button>
