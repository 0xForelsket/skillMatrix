...
import { db } from "@/db";
import { employeeSkills, employees, skills } from "@/db/schema";
	...
	for (const emp of emps) {
		...
		for (const skill of allSkills) {
			...
			for (const req of allReqs) {
				...
				const deptMatch =
					!req.departmentId || req.departmentId === emp.departmentId;
				...
				if (req.projectId) {
					projectMatch =
						emp.projects?.some((ep) => ep.projectId === req.projectId) ?? false;
				}
...
export async function getMatrixData(filters?: {
	...
	const emps = await db.query.employees.findMany({
		where: filters?.siteId ? eq(employees.siteId, filters.siteId) : undefined,
		with: {
			site: true,
			department: true,
			role: true,
